...
/** @file
 *
 * Headset Sample Application for 2070X devices.
 ...
 *
 * This application can run as a standalone Headset or in association with an Apps processor.
 * When running in standalone mode, it is required that 20706 controls the codec chip, Currently this is not implemented.
 * In association with an apps processor, this app serves to abstract the details of Bluetooth protocols and profiles while allowing Apps processor to deal with the business logic.
 * Apps processor is typically connected over UART and can send commands and receive notifications.
 * For an example of how this works with an apps processor, please refer to following applications in WICED-WiFi-SDK (43xxx_wi-Fi)
 *   bt_audio snippet app
 *   airplay_bt_combo_audio app
 *   hello_sensor app
 *   proximity_sensor app
 *
 ...
 * *->To use this application in conjunction with Apps processor:
 *  - Build application to produce a downloadable hcd file.  For example
 *    demo.headset-CYW920706WCDEVAL DIRECT_LOAD=1 build
 *  - The HCD file thus generated is downloaded into 20706 RAM. When using a 43xx- WICED platform,the HCD file is converted into an 'C' array of bytes,
 *  and the WICED platform software downloads this image.
 *  - Ensure that STANDALONE_HEADSET_APP flag is disabled.
 * TODO: The compilation of this app should be automated such a way that it is triggered when the MCU app is compiled.
 *       This eliminates the need to move the HCD file to MCU repository.
 *
...
const wiced_transport_cfg_t transport_cfg =
{
    ...
    {  WICED_TRANSPORT_UART_HCI_MODE, 3000000 },
...
void            hci_control_send_security_request_evt(uint8_t app_pairing_allowed );
...
void hci_control_init( void )
{
    ...
    /* This is platform specific. As of now hard coding it to enable Low power mode.*/
    wiced_sleep_config( WICED_TRUE, WICED_WAKE_GPIO_ACTIVE_LOW, WICED_WAKE_GPIO_ACTIVE_HIGH ); //sleep enable
}
    ...
    switch( event ) {
        ...
        case BTM_ENABLED_EVT:
            if( p_event_data->enabled.status != WICED_BT_SUCCESS ) {
                ...
                hci_status = HCI_CONTROL_STATUS_SUCCESS;
                ...
                if (p_key_info_pool == NULL) {
                    hci_status = HCI_CONTROL_STATUS_OUT_OF_MEMORY;
...
//          p_event_data->pairing_io_capabilities_br_edr_request.auth_req     = BTM_AUTH_ALL_PROFILES_NO;
        ...
        case BTM_SECURITY_REQUEST_EVT:
            hci_control_send_security_request_evt( hci_control_cb.pairing_allowed );
...
void hci_control_handle_set_local_name(uint8_t* p_data, uint32_t data_len)
{
    ...
    headset_write_eir(&p_data[1], p_data[0] - 1);
}
...
void hci_control_send_security_request_evt(uint8_t app_pairing_allowed )
{
    wiced_transport_send_data( HCI_CONTROL_EVENT_SECURITY_REQ, &app_pairing_allowed, 1);
}
...
int hci_control_write_nvram( int nvram_id, int data_len, void *p_data, wiced_bool_t from_host )
{
    ...
    uint8_t                   *p = tx_buf;
    hci_control_nvram_chunk_t *p1;
    hci_control_nvram_chunk_t *prev;
    wiced_result_t            result;
