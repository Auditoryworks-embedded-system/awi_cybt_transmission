...
static wiced_bt_gatt_status_t   hci_control_le_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data);
#ifdef ENABLE_HCI_TRACE
static void                     hci_control_le_trace_cback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
static void                     hci_control_le_set_advertisement_data( void );
static void                     hci_control_le_send_message( void );
static void                     hci_control_le_gatts_increment_notify_value( void );
static void                     hci_control_le_timeout( uint32_t count );
static void                     hci_control_le_fine_timeout( uint32_t finecount );
static void                     hci_control_le_smp_bond_result( uint8_t result );
static void                     hci_control_le_encryption_changed( wiced_result_t result, uint8_t* bd_addr );
static void                     hci_control_le_interrupt_handler( void* user_data, uint8_t pin );
static void                     hci_control_le_conn_idle_timeout ( uint32_t arg );
static void                     hci_control_le_load_keys_for_address_resolution( void );
...
void hci_control_le_init( void )
{
    ...
    wiced_hal_gpio_register_pin_for_interrupt( WICED_GPIO_BUTTON, hci_control_le_interrupt_handler, NULL );

    /* Register with stack to receive GATT callback */
    gatt_status = wiced_bt_gatt_register( hci_control_le_gatts_callback );
    ...
    wiced_bt_dev_register_hci_trace( hci_control_le_trace_cback );
#endif
    ...
    wiced_bt_app_start_timer( HELLO_SENSOR_APP_TIMEOUT_IN_SECONDS, HELLO_SENSOR_APP_FINE_TIMEOUT_IN_MS,
        hci_control_le_timeout, hci_control_le_fine_timeout );
    ...
    hci_control_le_load_keys_for_address_resolution();
 ...
 */
#ifdef ENABLE_HCI_TRACE
void hci_control_le_trace_cback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
    //send the trace
 ...
 * when it wakes up
 */
void hci_control_le_advertisement_stopped( void )
{
    wiced_result_t result;
 ...
 */
void hci_control_le_fine_timeout( uint32_t finecount )
{
    hello_sensor_state.fine_timer_count++;
    if (0 == (hello_sensor_state.fine_timer_count & 0x1f)) {
        WICED_BT_TRACE("hci_control_le_fine_timeout: %d\n", hello_sensor_state.fine_timer_count);
    }
}
...
void hci_control_le_smp_bond_result( uint8_t result )
{
 ...
 * send it out
 */
void hci_control_le_encryption_changed( wiced_result_t result, uint8_t* bd_addr )
{
    ...
    if ( ( !hello_sensor_state.flag_stay_connected ) && !hello_sensor_state.flag_indication_sent  ) {
        wiced_bt_app_start_conn_idle_timer( HELLO_SENSOR_CONN_IDLE_TIMEOUT_IN_SECONDS, hci_control_le_conn_idle_timeout );
    }
}
 ...
 */
void hci_control_le_interrupt_handler(void* data, uint8_t pin )
{
    ...
    hci_control_le_gatts_increment_notify_value();
 ...
 */
void hci_control_le_conn_idle_timeout ( uint32_t arg )
{
    WICED_BT_TRACE( "hci_control_le_conn_idle_timeout\n" );
 ...
 */
void hci_control_le_send_message( void )
{
    WICED_BT_TRACE( "hci_control_le_send_message: CCC:%d\n", hello_sensor_hostinfo.characteristic_client_configuration );
 ...
 */
attribute_t * hci_control_le_get_attribute( uint16_t handle )
{
    int i;
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
    attribute_t *puAttribute;
    int          attr_len_to_copy;

    if ( ( puAttribute = hci_control_le_get_attribute(p_read_data->handle) ) == NULL) {
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_write_exec_handler( uint16_t conn_id, wiced_bt_gatt_exec_flag_t exec_falg )
{
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_mtu_handler( uint16_t conn_id, uint16_t mtu)
{
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_conf_handler( uint16_t conn_id, uint16_t handle )
{
...
wiced_bt_gatt_status_t hci_control_le_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    wiced_result_t result;
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_connection_down( wiced_bt_gatt_connection_status_t *p_status )
{
    wiced_result_t result;
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_conn_status_cb( wiced_bt_gatt_connection_status_t *p_status )
{
    if ( p_status->connected ) {
        return hci_control_le_gatts_connection_up( p_status );
    }

    return hci_control_le_gatts_connection_down( p_status );
}

/*
 * Process GATT request from the peer
 */
wiced_bt_gatt_status_t hci_control_le_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_data )
{
    wiced_result_t result = WICED_BT_GATT_INVALID_PDU;

    WICED_BT_TRACE( "hci_control_le_gatts_req_cb. conn %d, type %d\n", p_data->conn_id, p_data->request_type );

    switch ( p_data->request_type ) {
    case GATTS_REQ_TYPE_READ:
        result = hci_control_le_gatts_req_read_handler( p_data->conn_id, &(p_data->data.read_req) );
        break;

    case GATTS_REQ_TYPE_WRITE:
        result = hci_control_le_gatts_req_write_handler( p_data->conn_id, &(p_data->data.write_req) );
        break;

    case GATTS_REQ_TYPE_WRITE_EXEC:
        result = hci_control_le_gatts_req_write_exec_handler( p_data->conn_id, p_data->data.exec_write );
        break;

    case GATTS_REQ_TYPE_MTU:
        result = hci_control_le_gatts_req_mtu_handler( p_data->conn_id, p_data->data.mtu );
        break;

    case GATTS_REQ_TYPE_CONF:
        result = hci_control_le_gatts_req_conf_handler( p_data->conn_id, p_data->data.handle );
        break;
 ...
 */
wiced_bt_gatt_status_t hci_control_le_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_INVALID_PDU;

    switch(event) {
    case GATT_CONNECTION_STATUS_EVT:
        result = hci_control_le_gatts_conn_status_cb( &p_data->connection_status );
        break;

    case GATT_ATTRIBUTE_REQUEST_EVT:
        result = hci_control_le_gatts_req_cb( &p_data->attribute_request );
        break;
 ...
 */
void hci_control_le_gatts_increment_notify_value( void )
{
...
static void hci_control_le_load_keys_for_address_resolution( void )
{
    ...
    WICED_BT_TRACE("hci_control_le_load_keys_for_address_resolution %B result:%d \n", p, result );
}
